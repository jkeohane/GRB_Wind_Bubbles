...
"""
Fit_GRB_user_medium_emcee_SPEED.py

Purpose:
  Faster turnaround by reducing data points and sampler steps,
  while keeping schema-aware CSV parsing and plotting.

Changes vs v7:
  • Log-time binning per frequency group (default: 8 bins per decade).
  • num_nu_grid = 32 (from 64).
  • burn = 100, nsteps = 300 (from 200/600).  nwalkers still 36.

You can tune TIME_BINS_PER_DECADE below.
"""
...
# ---------- CSV loader (same as v7, condensed) ----------
...
def logtime_bin_group(t, f, e, nbins):
    ...
    edges = np.linspace(lo, hi, max(2, min(nbins, tpos.size)))
    ...
    for b in range(edges.size-1):
        ...
        # combine errors as 1/sqrt(sum w)
...
def bin_data(t, nu, f, e):
    ...
    for g in groups:
        ...
        tb, fb, eb = logtime_bin_group(t[g], f[g], e[g], TIME_BINS_PER_DECADE*max(1,int(np.log10(t[g].max()/max(t[g].min(),1e-6))+1)))
